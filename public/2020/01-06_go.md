...
人脸识别模型：
    ...
    基于完全使用 Tensorflow 重新实现的 insightface 模型训练， softamx / arcface / center loss 损失函数训练

    重新优化的 Triplet 损失函数，lfw 等的验证数据集用于模型验证

封装移植：

    C++ 库调用 Tensorflow，加载模型用于人来年识别

    Go 语言调用 Tensroflow，加载模型测试，生成静态库

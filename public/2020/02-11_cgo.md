...
## MNN
  ...
  # cmake .. && make -j4
  cmake -DMNN_BUILD_DEMO=ON -DMNN_BUILD_CONVERTER=true .. && make -j4
  ...
  **Comparing Tflite**
  ...
  # 64.3 ms ± 3.55 ms per loop (mean ± std. dev. of 7 runs, 100 loops each)
  ```
## Opencv
  ```sh
...
## ncnn
  ...
  - **onnx -> ncnn**
    ...
    # 最后一个参数是 flag，0 表示 fp32，1 表示 fp16
    ./ncnnoptimize ncnn.param ncnn.bin ncnn-opt.param ncnn-opt.bin 1
    ```
## Tengine
...
## ncnn arm project
  ...
  - **Build**
    ...
    **Android run**
    ...
    adb shell 'mkdir -p /data/mobile_test; mkdir -p /data/mobile_test/test; mkdir -p /data/mobile_test/export/assets/mtcnn'
    adb push $HOME/Android/Sdk/ndk/21.0.6113669/toolchains/llvm/prebuilt/linux-x86_64/lib64/clang/9.0.8/lib/linux/arm/libomp.so \
        ...
        a.out /data/mobile_test
    adb push export/assets/mtcnn/* /data/mobile_test/export/assets/mtcnn
...
## TFlite project
  ...
  - **ARM64 env**
    ...
    ANDROID_CC="$HOME/Android/Sdk/ndk/20.0.5594570/toolchains/llvm/prebuilt/linux-x86_64/bin/aarch64-linux-android29-clang -Wl,-rpath-link,$HOME/Android/Sdk/ndk/20.0.5594570/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/aarch64-linux-android/29"
    ANDROID_ARCH="arm64"
    ```
  - **ARM32 env**
    ```sh
    ...
    ANDROID_CC="$HOME/Android/Sdk/ndk/20.0.5594570/toolchains/llvm/prebuilt/linux-x86_64/bin/armv7a-linux-androideabi29-clang -Wl,-rpath-link,$HOME/Android/Sdk/ndk/20.0.5594570/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/arm-linux-androideabi/29"
    ANDROID_CXX="$HOME/Android/Sdk/ndk/20.0.5594570/toolchains/llvm/prebuilt/linux-x86_64/bin/armv7a-linux-androideabi29-clang++ -Wl,-rpath-link,$HOME/Android/Sdk/ndk/20.0.5594570/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/arm-linux-androideabi/29"
    ANDROID_ARCH="arm"
    ```
  - **ARM gomobile bind**
    ```sh
  ...
  - **ARM binary**
    ```sh
    CGO_ENABLED=1 GOOS=android GOARCH=$ANDROID_ARCH CC=$ANDROID_CC CXX=$ANDROID_CXX go build tdFace.mobile/faceModelTflite
    CGO_ENABLED=1 GOOS=android GOARCH=$ANDROID_ARCH CC=$ANDROID_CC CXX=$ANDROID_CXX go build test/test_faceModelTflite.go
    ```
    **Run test**
    ```sh
    adb push test_faceModelTflite export/assets/faceModelTflite/model.tflite $HOME/workspace/tensorflow.arm32/bazel-bin/tensorflow/lite/c/libtensorflowlite_c.so /data

    adb shell 'mkdir -p /data/mobile_test/export/assets/faceModelTflite; mv /data/model.tflite /data/mobile_test/export/assets/faceModelTflite/'
    adb shell 'cd /data/mobile_test; chmod a+x ./test_faceModelTflite; LD_LIBRARY_PATH=./ ./test_faceModelTflite'
    # >>>> Threads: -1
...
## Yoloface
  ...
  - **ARM binary**
    ...
    ANDROID_CXX="$HOME/Android/Sdk/ndk/20.0.5594570/toolchains/llvm/prebuilt/linux-x86_64/bin/armv7a-linux-androideabi29-clang++ -Wl,-rpath-link,$HOME/Android/Sdk/ndk/20.0.5594570/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/arm-linux-androideabi/29"
    ANDROID_ARCH="arm"
    CGO_ENABLED=1 GOOS=android GOARCH=$ANDROID_ARCH CXX=$ANDROID_CXX CC=$ANDROID_CC go build test/test_yoloface.go

    adb shell 'mkdir -p /data/mobile_test; mkdir -p /data/mobile_test/test; mkdir -p /data/mobile_test/export/assets/yoloface'
    adb push $HOME/Android/Sdk/ndk/21.0.6113669/toolchains/llvm/prebuilt/linux-x86_64/lib64/clang/9.0.8/lib/linux/arm/libomp.so \
        $HOME/Android/Sdk/ndk/21.0.6113669/toolchains/llvm/prebuilt/linux-x86_64/sysroot/usr/lib/arm-linux-androideabi/libc++_shared.so \
        test_yoloface /data/mobile_test
...
- **Convert torch model to ONNX**
  ...
  image = torch.from_numpy(np.ones((1, 3, 224, 224)))
  image = image.to(device).float()
  net_preds = net(tim)

  output_onnx = 'mbv3_small_1.onnx'
  input_names = ["input0"]
...
- **ONNX inference**
  ...
  ibb = np.vstack([resize(image, (76, 160)), np.zeros([180, 160, 3])])
  onnx_preds = sess.run(None, {sess.get_inputs()[0].name:  np.expand_dims(np.transpose(ibb.astype('float32'), (2, 0, 1)), axis=0)})
  det = non_max_suppression(torch.from_numpy(np.concatenate(onnx_preds, 1)), 0.25, 0.35, multi_label=False, classes=0, agnostic=False,land=True ,point_num=5)[0].detach().numpy()
  bbs = det[:, :4].astype('int')
  plt.imshow(ibb)
  for bb in bbs:
      plt.plot([bb[0], bb[2], bb[2], bb[0], bb[0]], [bb[1], bb[1], bb[3], bb[3], bb[1]])
...
- **MNN inference**
  ...
  ```
  `net_preds` / `onnx_preds` / `mnn_preds` should be similar
- **yolodecode**
  ```cpp
  ...
  ```
  ```py
  tensor([ 4.08473e+00,  5.25838e+00,  1.58704e+01,  1.78815e+01, -1.47989e+01, -1.71405e+01,  8.98814e-01, -1.52191e+01, -7.64142e+00, -9.
10389e+00, -1.44117e+01, -1.32178e+00, -3.81882e+00,  1.06386e+00,  4.03128e-06,  5.63769e-01], grad_fn=<SelectBackward>)
  ```
  ```py
  pred_anchors = [
      [[12, 12], [20, 20], [32, 32]],
  ...
  def yolodecode(pred, pred_anchor, stride):
      ...
      anchors = torch.Tensor(pred_anchor)
      anchor_vec = anchors / stride
      anchor_wh = anchor_vec.view(1, na, 1, 1, 2)

      bs, _, ny, nx = pred.shape
      grid = create_grids((nx, ny), pred.device)
      # grid = torch.from_numpy(np_create_grids((nx, ny))).to(pred.device).float()
      anchor_vec, anchor_wh = anchor_vec.to(pred.device), anchor_wh.to(pred.device)
      ...
      for i in range(point_num):
          io[..., 4 + i*2:4 + i*2 + 2] = (io[..., 4 + i*2:4 + i*2 + 2] * anchor_wh) + grid

      io[..., :4 + point_num * 2] *= stride
      torch.sigmoid_(io[..., point_num * 2 + 4:])
      yolo_out = io.view(bs, -1, no)
      return yolo_out

  net_yolo_out = [yolodecode(pp, aa, ss) for pp, aa, ss in zip(net_preds, pred_anchors, strides)]
  net_yolo_out = torch.cat(net_yolo_out, 1)
  net_det = non_max_suppression(net_yolo_out, 0.25, 0.35, multi_label=False, classes=0, agnostic=False,land=True ,point_num=5)
  ```
  ```py
  def create_grids(ng=(13, 13), device='cpu'):
    nx, ny = ng  # x and y grid size
    ...
    # build xy offsets
    yv, xv = torch.meshgrid([torch.arange(ny, device=device), torch.arange(nx, device=device)])
    grid = torch.stack((xv, yv), 2).reshape((1, 1, ny, nx, 2)).float()
    return grid
  ...
  def yolodecode(pred, pred_anchor, stride):
    ...
    wh = pred[..., 2:4].clone()
    wh = torch.exp(wh) * anchor_wh
    ld1 = pred[..., 4:6].clone()
    ld1 = ld1 * anchor_wh + grid
    ld2 = pred[..., 6:8].clone()
    ld2 = ld2 * anchor_wh + grid
    ld3 = pred[..., 8:10].clone()
    ld3 = ld3 * anchor_wh + grid
    ld4 = pred[..., 10:12].clone()
    ld4 = ld4 * anchor_wh + grid
    ld5 = pred[..., 12:14].clone()
    ld5 = ld5 * anchor_wh + grid
    conf = pred[..., 14:].clone()
    conf = torch.sigmoid(conf)
    yolo_out = torch.cat((xy * stride, wh * stride, ld1 * stride, ld2 * stride, ld3 * stride, ld4 * stride, ld5 * stride, conf), -1)
    return yolo_out.view(bs, -1, no)
  ```
  ```py
  def sigmoid(xx):
  ...
  def np_yolodecode(pred, pred_anchor, stride):
      na = 3 # number of anchors (3)
      nc = 1 # number of classes (1)
      point_num = 5 # number of landmark points (5)
      no = nc + 5 + point_num * 2 # number of outputs (16)
      ...
      io = pred.copy()
      io = np.transpose(io.reshape(bs, na, no, ny, nx), (0, 1, 3, 4, 2))
      io[..., :2] = sigmoid(io[..., :2]) + grid
      io[..., 2:4] = np.exp(io[..., 2:4]) * anchor_wh
      for ii in range(point_num):
          io[..., 4 + ii*2 : 4 + (ii + 1)*2] = (io[..., 4 + ii*2 : 4 + (ii + 1)*2] * anchor_wh) + grid
      io[..., :4 + point_num * 2] *= stride
      io[..., 4 + point_num * 2:] = sigmoid(io[..., 4 + point_num * 2:])
      return io.reshape(bs, -1, no)

  onnx_yolo_out = [np_yolodecode(pp, aa, ss) for pp, aa, ss in zip(onnx_preds, pred_anchors, strides)]
  onnx_yolo_out = np.concatenate(onnx_yolo_out, 1)
  non_max_suppression(torch.from_numpy(onnx_yolo_out), 0.25, 0.35, multi_label=False, classes=0, agnostic=False,land=True ,point_num=5)[0].detach().numpy()
  ```
  ```py
  def yolodecode(pre, anchor, confidence_threshold, net_w, net_h) {
      w, h = pre.shape[-2:]
      for cc in anchor {
          channel_cur = cc * 16
          stridew, strideh = net_w / w, net_h / h

          for ii in range(h):
              for jj in range(w):
                  idx = ii * w + jj
                  confidence = sigmoid(pre)
...
## export
  ```sh
  CGO_ENABLED=1 GOOS=android GOARCH=$ANDROID_ARCH CXX=$ANDROID_CXX CC=$ANDROID_CC go build test/test_export.go
